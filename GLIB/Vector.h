...
namespace glib{
  ...
  {
  private:
      virtual void         _AssertStep( int step ) const;
      virtual void         _AssertRange( int index ) const;
      virtual unsigned int _TrueIndex( int index ) const;
      virtual void         _AssertDivision( const Vector<T>& V ) const;
      virtual void         _AssertDivision( T value ) const;
  ...
  template< class T > inline Vector<T> Vector<T>::Dot( const Matrix<T>& M ) const{ Vector W; for ( unsigned int i=0; i<this->Size(); ++i ) W.Append( this->Dot( M.Column(i) ) ); return W;}
